...
MinimaxChooser::MinimaxResult MinimaxChooser::minimax(
  ...
  if (winner != Board::INVALID) {
    if (winner == Board::RED) {
      return make_pair(MoveList{}, std::numeric_limits<double>::max());
    } else {
      return make_pair(MoveList{}, std::numeric_limits<double>::lowest());
    }
  }
  ...
  if (maximizing) {
    ...
    if (max_depth == 8 || max_depth == 7) {
      ...
      auto s = absl::StrJoin(candidates, "\n ", [](std::string *out, auto pr) {
        auto moves =
            absl::StrJoin(pr.first, ":", [](std::string *out, auto col) {
